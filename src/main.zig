...
pub fn main() !void {
    ...
    var ground_sphere = Sphere{ .center = zm.F32x4{ 0.0, -100.5, -1.0, 0.0 }, .radius = zm.f32x4s(100.0), .mat = material_ground.interface() };
    try world.add(ground_sphere.interface());
    ...
    var left_sphere_interior = Sphere{ .center = zm.F32x4{ -1.0, 0.0, -1.0, 0.0 }, .radius = zm.f32x4s(-0.4), .mat = material_left.interface() };
    try world.add(left_sphere_interior.interface());
    ...
    var camera: Camera = .{
        ...
        .rnd = rnd.random(),
        .vfov = 20,
        .lookfrom = zm.F32x4{ -2, 2, 1, 0 },
        .lookat = zm.F32x4{ 0, 0, -1, 0 },
        .defocus_angle = 10.0,
        .focus_dist = 3.4,
    };
